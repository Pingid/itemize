...
fn generate_hetero_impl_for_n(n: usize) -> String {
    ...
    let destructure = if n == 1 {
        format!("{},", destructure_vars.join(", "))
    } else {
        destructure_vars.join(", ")
    };
...
impl IntoItemsTuple {
    ...
    fn tuple_ty(&self, spec: &TupleSpec) -> String {
        let tuple_ty = spec.tuple_type_string();
        match self {
            Self::Owned => tuple_ty,
            Self::Ref { lifetime } => format!("&{lifetime} {tuple_ty}"),
            Self::RefRef { outer, inner } => format!("&{outer} &{inner} {tuple_ty}"),
        }
    }
    ...
    fn iter_type(&self, spec: &TupleSpec) -> String {
        match self {
            ...
            Self::RefRef { inner, .. } => {
                let tuple_ty = spec.tuple_type_string();
                format!("<&{inner} {tuple_ty} as IntoItems<T>>::IntoIter")
            }
        }
    }
    ...
    fn body(&self, spec: &TupleSpec) -> Vec<String> {
        match self {
            Self::Owned | Self::Ref { .. } => vec![
                format!("let {} = self;", spec.destructure_pattern()),
                format!(
                    "[{}].into_iter()",
    ...
    fn try_iter_type(&self, spec: &TupleSpec) -> String {
        match self {
            ...
            Self::RefRef { inner, .. } => {
                let tuple_ty = spec.tuple_type_string();
                format!("<&{inner} {tuple_ty} as TryIntoItems<T, E>>::IntoIter")
            }
        }
    }
...
impl RowBorrow {
    ...
    fn tuple_ty(&self, spec: &TupleSpec) -> String {
        let tuple_ty = spec.tuple_type_string();
        match self {
            RowBorrow::Owned => tuple_ty,
            RowBorrow::Ref { lifetime } => format!("&{lifetime} {tuple_ty}"),
        }
    }
...
impl TupleSpec {
    ...
    fn tuple_type_string(&self) -> String {
        let args = self.join_types(", ");
        if self.arity == 1 {
            format!("({args},)")
        } else {
            format!("({args})")
    ...
    fn destructure_pattern(&self) -> String {
        let values = self.join_values(", ");
        if self.arity == 1 {
            format!("({values},)")
        } else {
            format!("({values})")
